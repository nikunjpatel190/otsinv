...
//		$CI =& get_instance();
	 ...
	 */	
	function go_upload($field = 'userfile', $intRecordId="") {
        $CI =& get_instance(); 
...
//            echo "$fname\n\n<br><br>\n\n";
			...
			$arrFileInfo 		= 	pathinfo($_FILES[$field]['name'][$i]);
			$strExtension 		= 	$arrFileInfo['extension'];
			$strUploadFilename	=	$intRecordId."_".rand().".".$strExtension;
			
			$strOriginalFileName = 	$_FILES[$field]['name'][$i];
			
            $CI->upload->file_temp 	= 	$_FILES[$field]['tmp_name'][$i];
            $CI->upload->file_name 	= 	$strUploadFilename;
			...
			else 
			{
                if ($imageVar = $this->multiple_image_properties($CI->upload->upload_path.$CI->upload->file_name)) {
                    ...
                    $file_list[] = array(
							...
							'original_file_name' => $strOriginalFileName
                            );
                } else {
                    $file_list[] = array(
                            'name' => $CI->upload->file_name,
