    ...
    <div class="span12">
        <table width="100%" cellpadding="5" cellspacing="5" border="0" class="table table-striped table-bordered table-hover dataTable" id="pagelist_center">
            ...
            <tbody>
                ...
                {
                    foreach($rsUserTypes as $arrRecord)
                    {
                        $strEditLink	=	"index.php?c=user&m=AddUserTypes&action=E&id=".$arrRecord['u_typ_id'];
                        echo '<tr>';
						echo '<td><input type="checkbox" name="chk_lst_list1[]" id="chk_lst_'.$arrRecord['u_typ_id'].'" value="'.$arrRecord['u_typ_id'].'" /><span class="lbl"></span></td>';
						...
						echo '<td>'. $arrRecord['u_typ_code'] .'</td>';
                        echo '<td>'. $arrRecord['u_typ_desc'] .'</td>';
...
{
    window.location.href = 'index.php?c=user&m=addUserTypes&action=A';
}
...
function DeleteRow()
{
	...
	else
	{
		...
		if(responce==true)
		{
			$('#frm_list_record').attr('action','index.php?c=user&m=deleteUserTypes');
